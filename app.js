...
function renderMovPendientes(){
  ...
  // Simplificado: usar gastos sin fecha de pago o con estado pendiente como proxy
  const hoy = new Date();
  ...
  // Obligaciones estimadas: gastos del último mes como proxy de pendientes
  const mesAnt = `${hoy.getFullYear()}-${String(hoy.getMonth()).padStart(2,"0")||"12"}`;
  const obligaciones = sumBy(gastos.filter(g=>(fechaOrdenable(g.fecha)||"").startsWith(mesAnt)),"total");
  const cobertura    = obligaciones > 0 ? saldoDisp / obligaciones : null;

  const semaforoColor = cobertura===null?"var(--muted)":cobertura>=1.5?"var(--green)":cobertura>=1?"var(--amber)":"var(--red)";
  const semaforoLabel = cobertura===null?"Sin datos":cobertura>=1.5?"✅ Buena cobertura":cobertura>=1?"⚠️ Cobertura ajustada":"🔴 Cobertura insuficiente";
...
function renderControlFinanciero() {
  ...
  <style>
    ...
    .cf-sec-row td, .cf-tot-row td:first-child,
    .cf-ebitda-row td:first-child, .cf-result-row td:first-child,
    .cf-cajaacum-row td:first-child, .cf-financ-row td:first-child {
      position: sticky;
      left: 0;
      z-index: 2;
    }
    ...
    <div class="cf-table-wrap">
      <table class="cf-table">
        ...
        <tbody>
          ...
          ${totRow("Total financiamiento",   "financTotal",   acums.financTotal)}
          ${keyRow("Saldo proyecto (excedente / por financiar)", "saldoProyecto", acums.saldoProyecto, "cf-cajaacum-row")}
