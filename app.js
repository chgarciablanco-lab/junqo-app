...
function renderControlFinanciero() {
  ...
  // ── Aportes reales desde Supabase (ventasIngresos) por mes CF ──
  // CF usa nombres cortos: "Ene", "Feb", "Mar"... M0 = mes de inicio
  const MESES_CORTOS = ["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"];
  const aportesReales = {};

  // 1. Ingresos manuales desde Supabase (ingresos_junquillar)
  for (const ing of (typeof ventasIngresos !== "undefined" ? ventasIngresos : [])) {
    ...
    const mesIdx = Number(f.slice(5, 7)) - 1;
    const mesCorto = MESES_CORTOS[mesIdx] || "";
    if (!mesCorto) continue;
    aportesReales[mesCorto] = (aportesReales[mesCorto] || 0) + numberValue(ing.monto);
  }

  // 2. Abonos bancarios desde cartola (movimientos_banco)
  for (const mov of (typeof movBanco !== "undefined" ? movBanco : [])) {
    ...
    const mesCorto = MESES_CORTOS[mesIdx] || "";
    if (!mesCorto) continue;
    // Evitar duplicar si ya existe un ingreso manual con mismo monto y mes
    const yaExiste = (typeof ventasIngresos !== "undefined" ? ventasIngresos : []).some(ing =>
      fechaOrdenable(ing.fecha).slice(5, 7) === f.slice(5, 7) &&
      Math.abs(numberValue(ing.monto) - numberValue(mov.monto)) < 1
    );
    if (!yaExiste) {
      aportesReales[mesCorto] = (aportesReales[mesCorto] || 0) + numberValue(mov.monto);
    }
  }
  ...
  for (const m of ms) {
    ...
    // Aportes: reales desde Supabase + crédito hipotecario del demo
    ...
    <div class="cf-table-wrap">
      <table class="cf-table">
        ...
        <tbody>
          ...
          ${dataRow("Aportes de capital (Supabase)", m => C[m].aporteReal || 0)}
