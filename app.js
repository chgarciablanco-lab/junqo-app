...
function renderControlFinanciero() {
  ...
  /* ── Año del proyecto + filtro de meses ──────────────────── */
  const cfYear  = Number(localStorage.getItem("junqo_cf_year")  || new Date().getFullYear());
  ...
  for (const ing of (typeof ventasIngresos !== "undefined" ? ventasIngresos : [])) {
    ...
    const mesCorto = MESES_CORTOS[Number(f.slice(5,7)) - 1] || "";
    if (mesCorto) aportesReales[mesCorto] = (aportesReales[mesCorto] || 0) + numberValue(ing.monto);
    if (f.slice(0,7) === MES_KEY_TO_REAL["M0"])
      aportesReales["M0"] = (aportesReales["M0"] || 0) + numberValue(ing.monto);
  }
  for (const mov of (typeof movBanco !== "undefined" ? movBanco : [])) {
    ...
    if (!f || f.length < 7) continue;
    const mesCorto = MESES_CORTOS[Number(f.slice(5,7)) - 1] || "";
    if (mesCorto) aportesReales[mesCorto] = (aportesReales[mesCorto] || 0) + numberValue(mov.monto);
    if (f.slice(0,7) === MES_KEY_TO_REAL["M0"])
      aportesReales["M0"] = (aportesReales["M0"] || 0) + numberValue(mov.monto);
  }
  ...
  for (const m of ALL_MS) {
    ...
    const financ     = aporteReal + mv(d.creditoHipotecario, m);
    const cajaNeta   = resultado + financ;
    cajaAcum        += cajaNeta;
    C[m] = { ingresos, gastosOp, gastosAdm, ebitda, impuesto, resultado, financ, aporteReal, cajaNeta, cajaAcum };
  }
    ...
    <div class="card-header-row">
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--muted)">
          <span>Año</span>
          <input type="number" class="filter-select" style="height:34px;width:80px;font-size:12px"
            value="${cfYear}" min="2020" max="2035"
            onchange="localStorage.setItem('junqo_cf_year',this.value);renderControlFinanciero()"/>
          <span>Desde</span>
          ...
          <button class="filter-btn-clear" style="margin:0;height:34px;font-size:11px"
            onclick="localStorage.removeItem('junqo_cf_year');localStorage.removeItem('junqo_cf_desde');localStorage.removeItem('junqo_cf_hasta');renderControlFinanciero()">↺</button>
        </div>
      </div>
    </div>
