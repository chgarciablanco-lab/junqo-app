...
function renderControlFinanciero() {
  ...
  /* ── Filtro de meses ─────────────────────────────────────── */
  // Detectar año del proyecto desde los datos reales
  const _allFechas = [
  ...
  const cfYear = _allFechas.length
    ? Number(_allFechas[0].slice(0,4))
    : new Date().getFullYear();

  const cfDesde = localStorage.getItem("junqo_cf_desde") || "M0";
  const cfHasta = localStorage.getItem("junqo_cf_hasta") || "Dic";

  // Mapear cada columna a YYYY-MM para labels y filtro
  const MES_KEY_TO_REAL = {
    M0:  `${cfYear-1}-12`,
    Ene: `${cfYear}-01`, Feb: `${cfYear}-02`, Mar: `${cfYear}-03`,
    Abr: `${cfYear}-04`, May: `${cfYear}-05`, Jun: `${cfYear}-06`,
    Jul: `${cfYear}-07`, Ago: `${cfYear}-08`, Sep: `${cfYear}-09`,
    Oct: `${cfYear}-10`, Nov: `${cfYear}-11`, Dic: `${cfYear}-12`,
  };
  const MES_LABEL_MAP = {
    M0:"Mes 0", Ene:"Ene", Feb:"Feb", Mar:"Mar", Abr:"Abr",
    May:"May",  Jun:"Jun", Jul:"Jul", Ago:"Ago", Sep:"Sep",
    Oct:"Oct",  Nov:"Nov", Dic:"Dic",
  };
  const MESES_CORTOS = ["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"];

  // Filtrar columnas según rango elegido
  const ALL_MS = ["M0","Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"];
  const idxDesde = ALL_MS.indexOf(cfDesde);
  const idxHasta = ALL_MS.indexOf(cfHasta);
  const ms = ALL_MS.slice(
    idxDesde >= 0 ? idxDesde : 0,
    idxHasta >= 0 ? idxHasta + 1 : ALL_MS.length
  );

  // Label de columna: "Dic 2024", "Ene 2026", etc.
  const colLabel = m => {
    const ym = MES_KEY_TO_REAL[m];
    if (!ym) return m;
    const [y, mo] = ym.split("-");
    return `${MESES_CORTOS[Number(mo)-1]} ${y}`;
  };

  /* ── Aportes reales desde Supabase por mes CF ─────────────── */
  const aportesReales   = {}; // Aporte construcción
  const aportesTerreno  = {}; // Aporte terreno

  const addAporte = (obj, ym, monto) => {
    const mesCorto = MESES_CORTOS[Number(ym.slice(5,7)) - 1] || "";
    if (mesCorto) obj[mesCorto] = (obj[mesCorto] || 0) + monto;
    if (ym === MES_KEY_TO_REAL["M0"]) obj["M0"] = (obj["M0"] || 0) + monto;
  };

  for (const ing of (typeof ventasIngresos !== "undefined" ? ventasIngresos : [])) {
    if (ing.estado !== "Recibido") continue;
    const f = fechaOrdenable(ing.fecha);
    if (!f || f.length < 7) continue;
    const cat = (ing.categoria_contable || "").toLowerCase();
    if (cat.includes("terreno")) addAporte(aportesTerreno, f.slice(0,7), numberValue(ing.monto));
    else addAporte(aportesReales, f.slice(0,7), numberValue(ing.monto));
  }
  for (const mov of (typeof movBanco !== "undefined" ? movBanco : [])) {
    if (mov.tipo !== "abono") continue;
    const f = fechaOrdenable(mov.fecha);
    if (!f || f.length < 7) continue;
    addAporte(aportesReales, f.slice(0,7), numberValue(mov.monto));
  }

  /* ── Cálculos por mes ─────────────────────────────────────── */
  let cajaAcum = 0;
  const C = {};
  for (const m of ALL_MS) {
    ...
    const aporteReal    = aportesReales[m]  || 0;
    const aporteTerreno = aportesTerreno[m] || 0;
    const financ        = aporteReal + aporteTerreno + mv(d.creditoHipotecario, m);
    const cajaNeta   = resultado + financ;
    cajaAcum        += cajaNeta;
    C[m] = { ingresos, gastosOp, gastosAdm, ebitda, impuesto, resultado, financ, aporteReal, aporteTerreno, cajaNeta, cajaAcum };
  }

  /* ── KPI acumulados (del rango visible) ───────────────────── */
  ...
  /* ── Opciones para selectores de mes ─────────────────────── */
  const mesOpts = (sel) => ALL_MS.map(m =>
    `<option value="${m}"${m===sel?" selected":""}>${colLabel(m)}</option>`
  ).join("");
    ...
    <div class="card-header-row">
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--muted)">
          <span>Desde</span>
          <select class="filter-select" style="height:34px;font-size:12px"
          ...
          <button class="filter-btn-clear" style="margin:0;height:34px;font-size:11px"
            onclick="localStorage.removeItem('junqo_cf_desde');localStorage.removeItem('junqo_cf_hasta');renderControlFinanciero()">↺</button>
        </div>
      </div>
    </div>
      ...
      <table class="cf-table">
        <thead>
          <tr>
            ...
            ${ms.map(m => `<th class="cf-th-month">${colLabel(m)}</th>`).join("")}
          </tr>
        </thead>
        <tbody>
          ${secRow("INGRESOS")}
          ...
          ${dataRow("Aporte capital terreno", m => C[m].aporteTerreno || 0)}
          ${dataRow("Aporte de capital", m => C[m].aporteReal || 0)}
