...
const views = {
  ...
  caja:        { title:"Movimientos",         subtitle:"Saldo bancario, IVA y operación diaria del proyecto", visible:["section-caja"] },
  presupuestos: { title:"Presupuestos", subtitle:"Presupuesto por categoría — avance vs ejecutado", visible:["section-presupuestos"] },
  ...
  ventas:       { title:"Aportes e Ingresos", subtitle:"Aportes de capital e ingresos del proyecto",          visible:["section-ventas"] },
...
function renderKPIs(){
  ...
  const t=getTotals();
  // Usar presupuestos de Supabase si existen, si no usar PROJECT_BUDGET
  const totalPresupuesto = (typeof presupuestos !== "undefined" && presupuestos.length > 0)
    ? presupuestos.reduce((s,p)=>s+numberValue(p.presupuesto_neto),0)
    : PROJECT_BUDGET;
  const avance = totalPresupuesto ? (t.neto/totalPresupuesto)*100 : 0;
  const saldo  = Math.max(totalPresupuesto-t.neto, 0);
  const chipStyle=(bg,color)=>`background:${bg};color:${color};border:1px solid ${color}33`;
  el.innerHTML=[
    {title:"Avance financiero",    value:formatoPct(avance),      chip:["Ejecutado",chipStyle(avance>=90?"#fee2e2":avance>=70?"#fef3c7":"#d1fae5",avance>=90?"#dc2626":avance>=70?"#d97706":"#059669")], footer:`${formatoCLP(t.neto)} ejecutado de ${formatoCLP(totalPresupuesto)}`},
...
/* ── SALDO BANCARIO ─────────────────────────────────────── */
function renderMovSaldoBancario(){
  const el = $("caja-kpis");
  if(!el) return;

  const banco = typeof movBanco !== "undefined" ? movBanco : [];
  const totalAbonos = banco.filter(m=>m.tipo==="abono").reduce((s,m)=>s+numberValue(m.monto),0);
  const totalCargos = banco.filter(m=>m.tipo==="cargo").reduce((s,m)=>s+numberValue(m.monto),0);
  const saldoInicial = 0; // configurable en el futuro
  const saldoDisp    = saldoInicial + totalAbonos - totalCargos;

  // Movimientos del mes actual
  const hoy   = new Date();
  const mesAct = `${hoy.getFullYear()}-${String(hoy.getMonth()+1).padStart(2,"0")}`;
  const cobradoMes = banco.filter(m=>m.tipo==="abono"&&(m.fecha||"").startsWith(mesAct)).reduce((s,m)=>s+numberValue(m.monto),0);
  const pagadoMes  = banco.filter(m=>m.tipo==="cargo"&&(m.fecha||"").startsWith(mesAct)).reduce((s,m)=>s+numberValue(m.monto),0);

  el.innerHTML = `
  <div class="kpi-card">
    <div class="kpi-title">Ingresos recibidos</div>
    <div class="kpi-value" style="color:var(--green)">${formatoCLP(totalAbonos)}</div>
    <div class="kpi-footer">Total abonos bancarios</div>
  </div>
  <div class="kpi-card">
    <div class="kpi-title">Pagos realizados</div>
    <div class="kpi-value" style="color:var(--red)">${formatoCLP(totalCargos)}</div>
    <div class="kpi-footer">Total cargos bancarios</div>
  </div>
  <div class="kpi-card">
    <div class="kpi-title">Saldo disponible</div>
    <div class="kpi-value" style="color:${saldoDisp>=0?"var(--brand)":"var(--red)"}">${formatoCLP(saldoDisp)}</div>
    <div class="kpi-footer">Ingresos − pagos acumulados</div>
  </div>
  <div class="kpi-card">
    <div class="kpi-title">Este mes</div>
    <div class="kpi-value" style="font-size:16px">
      <span style="color:var(--green)">+${formatoCLP(cobradoMes)}</span>
      <span style="color:var(--muted);font-size:12px"> / </span>
      <span style="color:var(--red)">-${formatoCLP(pagadoMes)}</span>
    </div>
    <div class="kpi-footer">Cobrado / Pagado</div>
  </div>`;
}

/* ── IVA ─────────────────────────────────────────────────── */
function renderMovIVA(){
  const el = $("caja-control");
  if(!el) return;

  const ivaCredito   = sumBy(gastos.filter(g=>g.creditoFiscal!==false), "iva");
  const ivaDebito    = 0; // sin ventas aún
  const ivaNeto      = ivaDebito - ivaCredito;
  const estadoIva    = ivaNeto > 0
    ? `<span style="color:var(--red);font-weight:600">IVA por pagar</span>`
    : `<span style="color:var(--green);font-weight:600">Crédito acumulado</span>`;

  el.innerHTML = `
  <div class="card" style="margin-bottom:16px">
    <div class="card-title">IVA — Control tributario</div>
    <div class="card-sub">IVA Débito − IVA Crédito = IVA Neto estimado</div>
    <div class="kpi-grid" style="margin-top:14px">
      <div class="kpi-card">
        <div class="kpi-title">IVA Débito</div>
        <div class="kpi-value">${formatoCLP(ivaDebito)}</div>
        <div class="kpi-footer">De ventas realizadas</div>
      </div>
      <div class="kpi-card">
        <div class="kpi-title">IVA Crédito fiscal</div>
        <div class="kpi-value" style="color:var(--brand)">${formatoCLP(ivaCredito)}</div>
        <div class="kpi-footer">Acumulado histórico</div>
      </div>
      <div class="kpi-card">
        <div class="kpi-title">IVA Neto estimado</div>
        <div class="kpi-value" style="color:${ivaNeto>0?"var(--red)":"var(--green)"}">${formatoCLP(Math.abs(ivaNeto))}</div>
        <div class="kpi-footer">${estadoIva}</div>
      </div>
    </div>
  </div>`;
}
...
function renderMovPendientes(){
  ...
  const hoy = new Date();
  const banco = typeof movBanco!=="undefined"?movBanco:[];
  const saldoDisp = banco.filter(m=>m.tipo==="abono").reduce((s,m)=>s+numberValue(m.monto),0)
                  - banco.filter(m=>m.tipo==="cargo").reduce((s,m)=>s+numberValue(m.monto),0);

  // Obligaciones reales: gastos sin movimiento bancario asociado (pendientes de pago)
  // Como proxy: cargos banco que no tienen gasto registrado (diferencia banco - gastos)
  const mesActualYM = `${hoy.getFullYear()}-${String(hoy.getMonth()+1).padStart(2,"0")}`;
  const gastosRegistrados = sumBy(gastos, "total");
  const cargosTotal = banco.filter(m=>m.tipo==="cargo").reduce((s,m)=>s+numberValue(m.monto),0);
  // Obligaciones = diferencia positiva entre cargos banco y gastos con doc (lo que falta registrar)
  const obligaciones = Math.max(0, cargosTotal - gastosRegistrados);

  let cobertura = null;
  let semaforoColor, semaforoLabel;

  if (obligaciones === 0) {
    semaforoColor = "var(--muted)";
    semaforoLabel = "Sin obligaciones registradas";
  } else {
    cobertura = saldoDisp / obligaciones;
    semaforoColor = cobertura>=1.5?"var(--green)":cobertura>=1?"var(--amber)":"var(--red)";
    semaforoLabel = cobertura>=1.5?"✅ Buena cobertura":cobertura>=1?"⚠️ Cobertura ajustada":"🔴 Cobertura insuficiente";
  }

  el.innerHTML = `
  <div class="card" style="margin-bottom:16px">
    <div class="card-title">Pendientes e indicadores</div>
    <div class="card-sub">Cobertura = Saldo disponible / Obligaciones estimadas</div>
    <div class="kpi-grid" style="margin-top:14px">
      <div class="kpi-card">
        <div class="kpi-title">Cobertura</div>
        <div class="kpi-value" style="color:${semaforoColor}">${cobertura!==null?cobertura.toFixed(2)+"x":"—"}</div>
        <div class="kpi-footer">${semaforoLabel}</div>
      </div>
      <div class="kpi-card">
        <div class="kpi-title">Saldo disponible</div>
        <div class="kpi-value">${formatoCLP(saldoDisp)}</div>
        <div class="kpi-footer">Banco</div>
      </div>
      <div class="kpi-card">
        <div class="kpi-title">Crédito IVA acumulado</div>
        <div class="kpi-value" style="color:var(--brand)">${formatoCLP(sumBy(gastos,"iva"))}</div>
        <div class="kpi-footer">Recuperable al vender</div>
      </div>
    </div>
  </div>`;
}
...
let movTablaFiltro = "todo";

function renderMovTabla(){
  const el = $("caja-mensual");
  if(!el) return;

  // Construir filas: gastos + movBanco
  const banco = typeof movBanco!=="undefined"?movBanco:[];
  let rows = [];

  // Gastos → Pagos del proyecto (impactan banco e IVA)
  for(const g of gastos){
    rows.push({
      fecha:       g.fecha,
      tipo:        "Pago",
      documento:   g.tipo_documento||"—",
  ...
  // Cartola bancaria → solo abonos (ingresos recibidos)
  for(const m of banco){
    if(m.tipo !== "abono") continue;
    rows.push({
      fecha:       m.fecha,
      tipo:        "Ingreso",
      documento:   m.descripcion||"—",
      proveedor:   "—",
      neto:        m.monto||0,
      iva:         0,
      total:       m.monto||0,
  ...
  rows.sort((a,b)=>(fechaOrdenable(b.fecha)||"").localeCompare(fechaOrdenable(a.fecha)||""));

  // Aplicar filtro
  if(movTablaFiltro==="ingresos") rows=rows.filter(r=>r.tipo==="Ingreso");
  else if(movTablaFiltro==="pagos") rows=rows.filter(r=>r.tipo==="Pago"||r.tipo==="Gasto");
  else if(movTablaFiltro==="iva")   rows=rows.filter(r=>r.impactaIVA);
  else if(movTablaFiltro==="pendientes") rows=rows.filter(r=>r.estado==="pendiente");

  const filtros = [
    {k:"todo",      lbl:"Todo"},
    {k:"ingresos",  lbl:"Solo ingresos"},
    {k:"pagos",     lbl:"Solo pagos"},
    {k:"iva",       lbl:"Solo IVA"},
    {k:"pendientes",lbl:"Solo pendientes"},
  ];
  ...
  <div class="card">
    <div class="card-header-row">
      <div>
        <div class="card-title">Movimientos del proyecto</div>
        <div class="card-sub">Gastos registrados + movimientos bancarios</div>
      </div>
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        ${filtros.map(f=>`
          <button class="filter-btn-apply" style="padding:6px 12px;font-size:11px;${movTablaFiltro===f.k?"":"background:var(--bg-card);color:var(--text);border:1px solid var(--border)"}"
            onclick="movTablaFiltro='${f.k}';renderMovTabla()">${f.lbl}</button>
        `).join("")}
      </div>
    </div>
    <div class="table-wrap" style="margin-top:14px;overflow-x:auto">
      <div class="table-head" style="grid-template-columns:90px 70px 1fr 1fr 100px 80px 100px 80px 80px 60px">
        <div>Fecha</div><div>Tipo</div><div>Documento</div><div>Proveedor</div>
        <div style="text-align:right">Neto</div><div style="text-align:right">IVA</div>
        <div style="text-align:right">Total</div><div>Estado</div>
        <div>Banco</div><div>IVA</div>
      </div>
      <div>
        ${rows.length ? rows.slice(0,100).map(r=>`
          <div class="table-row" style="grid-template-columns:90px 70px 1fr 1fr 100px 80px 100px 80px 80px 60px">
            <div style="font-size:12px">${normalizarFecha(r.fecha)}</div>
            <div>
              <span class="jv-badge" style="font-size:10px;background:${r.tipo==="Ingreso"?"var(--green-soft)":r.tipo==="Pago"?"var(--red-soft)":"#f8fafc"};color:${r.tipo==="Ingreso"?"var(--green)":r.tipo==="Pago"?"var(--red)":"var(--muted)"}">
                ${r.tipo}
              </span>
            </div>
            <div class="doc-name" style="font-size:12px">${r.documento}</div>
            <div style="font-size:12px;color:var(--muted)">${r.proveedor}</div>
            <div style="text-align:right;font-size:12px">${formatoCLP(r.neto)}</div>
            <div style="text-align:right;font-size:12px;color:var(--brand)">${r.iva?formatoCLP(r.iva):"—"}</div>
            <div style="text-align:right;font-size:12px;font-weight:600">${formatoCLP(r.total)}</div>
            <div style="font-size:11px;color:var(--muted)">${r.estado}</div>
            <div style="text-align:center">${r.impactaBanco?"✅":"—"}</div>
            <div style="text-align:center">${r.impactaIVA?"✅":"—"}</div>
          </div>`).join("")
...
window.movTablaFiltro    = movTablaFiltro;
...
const cfDemoData = {
  ...
  gastosAdm: [
    ...
    {label:"Comercialización / corretaje",   M0:0,      Ene:0,     Feb:0,     Mar:0,     Abr:0,May:0,Jun:0,Jul:0,Ago:0,Sep:0,Oct:0,Nov:0,Dic:0},
    {label:"Otros administrativos",          M0:0,      Ene:0,     Feb:0,     Mar:0,     Abr:0,May:0,Jun:0,Jul:0,Ago:0,Sep:0,Oct:0,Nov:0,Dic:0},
  ],
...
function renderControlFinanciero() {
  ...
  const ymLabel = ym => { const[y,m]=ym.split("-"); return `${MESES_LABEL[Number(m)-1]} ${y}`; };
  const hoy    = new Date();
  ...
  // Solo abonos explícitamente clasificados como capital (sin default)
  const CLASIF_CF = ["Aporte de capital","Préstamo recibido"];
  for(const mov of (typeof movBanco!=="undefined"?movBanco:[])){
    if(mov.tipo!=="abono") continue;
    if(!mov.clasificacion || !CLASIF_CF.includes(mov.clasificacion)) continue;
    const ym=(fechaOrdenable(mov.fecha)||"").slice(0,7);
    if(ym) addAp(aportesReales,ym,numberValue(mov.monto));
  }

  /* ── Cálculos por clave CF ────────────────────────────────── */
  let cajaAcum = 0;
  const C = {};
  for(const m of CF_KEYS){
    const ingresos    = mv(d.ventaCasa,m)+mv(d.otrosIngresos,m);
    const costoDir    = d.gastosOp.reduce( (s,r)=>s+mv(r,m),0);
    const gastosAdm   = d.gastosAdm.reduce((s,r)=>s+mv(r,m),0);
    const resultadoAI = ingresos - costoDir - gastosAdm;
    const impuesto    = resultadoAI > 0 ? mv(d.impuesto,m) : 0;
    const resultado   = resultadoAI - impuesto;
    const apTerreno   = aportesTerreno[m] || 0;
    const apCapital   = aportesReales[m]  || 0;
    const financExterno = mv(d.creditoHipotecario,m);
    const financPropios = apTerreno + apCapital;
    const financTotal   = financPropios + financExterno;
    const costoTotal    = costoDir + gastosAdm;
    const saldoProyecto = financTotal - costoTotal;
    const cajaNeta      = resultado + financTotal;
    cajaAcum           += cajaNeta;
    C[m] = { ingresos, costoDir, gastosAdm, resultadoAI, impuesto, resultado,
              apTerreno, apCapital, financPropios, financExterno, financTotal,
              costoTotal, saldoProyecto, cajaNeta, cajaAcum };
  }

  /* ── Acumulados del rango visible ────────────────────────── */
  const visKeys = visibleYMs.map(ym=>ymToKey[ym]).filter(Boolean);
  const acum = fn => visKeys.reduce((s,m) => s + (fn(m)||0), 0);

  const acums = {
    ingresos:     acum(m => C[m].ingresos),
    costoDir:     acum(m => C[m].costoDir),
    gastosAdm:    acum(m => C[m].gastosAdm),
    resultadoAI:  acum(m => C[m].resultadoAI),
    impuesto:     acum(m => C[m].impuesto),
    resultado:    acum(m => C[m].resultado),
    apTerreno:    acum(m => C[m].apTerreno),
    apCapital:    acum(m => C[m].apCapital),
    financPropios:acum(m => C[m].financPropios),
    financExterno:acum(m => C[m].financExterno),
    financTotal:  acum(m => C[m].financTotal),
    costoTotal:   acum(m => C[m].costoTotal),
  };
  acums.saldoProyecto = acums.financTotal - acums.costoTotal;

  const mesHasta = visibleYMs.length ? ymLabel(visibleYMs[visibleYMs.length-1]) : "—";

  /* ── KPIs ─────────────────────────────────────────────────── */
  const kpiClr = v => v>0?"var(--green)":v<0?"var(--red)":"var(--muted)";
  ...
  /* ── Celda acumulado (sticky, fondo destacado) ────────────── */
  const fmtAcum = v => {
    if(!v||v===0) return `<span class="cf-zero">$0</span>`;
    const s = Math.abs(Math.round(v)).toLocaleString("es-CL");
    return v<0
      ? `<span style="color:var(--red);font-weight:700">-$${s}</span>`
      : `<span style="color:var(--green);font-weight:700">$${s}</span>`;
  };
  const acumCell = v =>
    `<td class="cf-num cf-acum-cell">${fmtAcum(v)}</td>`;

  /* ── Constructores de filas (Concepto | Acumulado | Mes...) ─ */
  const nCols = visibleYMs.length + 2; // +1 concepto +1 acumulado
  const secRow = lbl =>
    `<tr class="cf-sec-row"><td colspan="${nCols}">${lbl}</td></tr>`;
  const subRow = lbl =>
    `<tr class="cf-sub-row"><td colspan="${nCols}" style="font-size:11px;color:var(--muted);padding:4px 12px">${lbl}</td></tr>`;
  const dataRow = (lbl, fn, acumVal, cls="") =>
    `<tr class="cf-data-row${cls?" "+cls:""}">` +
    `<td class="cf-lbl">${lbl}</td>` +
    acumCell(acumVal) +
    visibleYMs.map(ym=>{const k=ymToKey[ym];return`<td class="cf-num">${fmt(k?fn(k):0)}</td>`;}).join("") +
    `</tr>`;
  const totRow = (lbl, key, acumVal, cls="cf-tot-row") =>
    `<tr class="${cls}">` +
    `<td class="cf-lbl">${lbl}</td>` +
    acumCell(acumVal) +
    visibleYMs.map(ym=>{const k=ymToKey[ym];return`<td class="cf-num">${fmt(k?C[k][key]:0)}</td>`;}).join("") +
    `</tr>`;
  const keyRow = (lbl, key, acumVal, cls) =>
    `<tr class="${cls}">` +
    `<td class="cf-lbl">${lbl}</td>` +
    acumCell(acumVal) +
    visibleYMs.map(ym=>{const k=ymToKey[ym];return`<td class="cf-num">${fmt(k?C[k][key]:0)}</td>`;}).join("") +
    `</tr>`;
  const spacer = () =>
    `<tr class="cf-spacer-row"><td colspan="${nCols}"></td></tr>`;

  /* ── Indicador saldo ──────────────────────────────────────── */
  const saldoLabel = acums.saldoProyecto >= 0
    ? `<span style="color:var(--green);font-weight:600">Excedente disponible</span>`
    : `<span style="color:var(--red);font-weight:600">Saldo por financiar</span>`;

  el.innerHTML = `
  <style>
    /* ── Contenedor scrollable ──────────────────────────────── */
    #cf-root .cf-table-wrap {
      overflow: auto;
      max-height: 68vh;
    ...
    /* ── Tabla con separación de celdas para sticky ─────────── */
    #cf-root .cf-table {
    ...
    /* ── STICKY: fila de encabezados (scroll vertical) ───────── */
    #cf-root .cf-table thead th {
      position: sticky;
      top: 0;
      z-index: 10;
      background: #f8fafc;
    ...
    /* ── STICKY: columna Concepto (scroll horizontal) ────────── */
    #cf-root .cf-table td:first-child,
    #cf-root .cf-table th:first-child {
      position: sticky;
      left: 0;
      z-index: 6;
      min-width: 200px;
    ...
    #cf-root .cf-table thead th:first-child {
      z-index: 14;
      background: #f8fafc;
    }
    /* ── STICKY: columna Acumulado ───────────────────────────── */
    #cf-root .cf-table td:nth-child(2),
    #cf-root .cf-table th:nth-child(2) {
      position: sticky;
      left: 200px;
      z-index: 6;
      min-width: 130px;
    ...
    #cf-root .cf-table thead th:nth-child(2) {
      z-index: 14;
      background: #dcfce7;
      font-size: 11px;
    ...
    /* ── Backgrounds por tipo de fila ───────────────────────── */
    #cf-root .cf-sec-row td            { background: #f1f5f9 !important; font-weight: 700; }
    #cf-root .cf-tot-row td:first-child,
    #cf-root .cf-tot-row td:nth-child(2) { background: #f8fafc !important; }
    #cf-root .cf-ebitda-row td:first-child,
    #cf-root .cf-ebitda-row td:nth-child(2) { background: #eff6ff !important; }
    #cf-root .cf-result-row td:first-child,
    #cf-root .cf-result-row td:nth-child(2) { background: #f0fdf4 !important; }
    #cf-root .cf-cajaacum-row td:first-child,
    #cf-root .cf-cajaacum-row td:nth-child(2) { background: #fefce8 !important; }
    /* ── Evitar solapamiento de sombra en scroll ─────────────── */
    #cf-root .cf-table td:first-child { box-shadow: 2px 0 4px -2px rgba(0,0,0,0.06); }
    #cf-root .cf-table td:nth-child(2) { box-shadow: 2px 0 4px -2px rgba(0,0,0,0.04); }
  </style>

  <div class="card">
    <div class="card-header-row" style="flex-wrap:wrap;gap:12px">
      <div>
        <div class="card-title">Control Financiero — Sin IVA</div>
        <div class="card-sub">Análisis del proyecto: ¿Va bien o mal?
          &nbsp;·&nbsp;
          <span style="color:var(--green);font-weight:600">Acumulado hasta ${mesHasta}</span>
        </div>
      </div>
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:12px;color:var(--muted)">
        ...
        <span>Hasta</span>
        <input type="month" class="filter-select" style="height:34px;font-size:12px" value="${cfHasta}"
          onchange="localStorage.setItem('junqo_cf_hasta',this.value);renderControlFinanciero()"/>
        <button class="filter-btn-clear" style="margin:0;height:34px;font-size:11px"
          onclick="localStorage.removeItem('junqo_cf_inicio');localStorage.removeItem('junqo_cf_desde');localStorage.removeItem('junqo_cf_hasta');renderControlFinanciero()">↺</button>
      </div>
    </div>

    <div class="cf-kpi-grid">
      <div class="cf-kpi-card">
        <div class="cf-kpi-lbl">Ingresos operacionales</div>
        <div class="cf-kpi-val" style="color:var(--brand)">${fmtKpi(acums.ingresos)}</div>
        <div class="cf-kpi-sub">Acumulado · Venta casa + otros</div>
      </div>
      <div class="cf-kpi-card">
        <div class="cf-kpi-lbl">Costo total proyecto</div>
        <div class="cf-kpi-val" style="color:var(--red)">${fmtKpi(acums.costoTotal)}</div>
        <div class="cf-kpi-sub">Acumulado · Directo + administrativo</div>
      </div>
      <div class="cf-kpi-card">
        <div class="cf-kpi-lbl">Resultado neto</div>
        <div class="cf-kpi-val" style="color:${kpiClr(acums.resultado)}">${fmtKpi(acums.resultado)}</div>
        <div class="cf-kpi-sub">Acumulado · Después de impuesto</div>
      </div>
      <div class="cf-kpi-card">
        <div class="cf-kpi-lbl">Saldo proyecto</div>
        <div class="cf-kpi-val" style="color:${kpiClr(acums.saldoProyecto)}">${fmtKpi(acums.saldoProyecto)}</div>
        <div class="cf-kpi-sub">${saldoLabel}</div>
      </div>
    </div>
    ...
    <div class="cf-table-wrap">
      <table class="cf-table">
        <thead>
          <tr>
            <th class="cf-th-lbl" style="position:sticky;left:0;z-index:3;background:#f8fafc">Concepto</th>
            <th class="cf-th-month cf-acum-header">Acumulado</th>
            ${visibleYMs.map(ym=>`<th class="cf-th-month">${ymLabel(ym)}</th>`).join("")}
          </tr>
        </thead>
        <tbody>

          ${secRow("INGRESOS OPERACIONALES")}
          ${dataRow("Venta casa",              m=>mv(d.ventaCasa,m),      acum(m=>mv(d.ventaCasa,m)))}
          ${dataRow("Otros ingresos operacionales", m=>mv(d.otrosIngresos,m), acum(m=>mv(d.otrosIngresos,m)))}
          ${totRow("Total ingresos", "ingresos", acums.ingresos)}

          ${secRow("COSTOS DIRECTOS DEL PROYECTO")}
          ${d.gastosOp.map(r=>dataRow(r.label, m=>mv(r,m), acum(m=>mv(r,m)))).join("")}
          ${totRow("Total costo directo", "costoDir", acums.costoDir)}

          ${secRow("GASTOS INDIRECTOS / ADMINISTRATIVOS")}
          ${d.gastosAdm.map(r=>dataRow(r.label, m=>mv(r,m), acum(m=>mv(r,m)))).join("")}
          ${totRow("Total gastos administrativos", "gastosAdm", acums.gastosAdm)}

          ${spacer()}
          ${keyRow("EBITDA / Resultado antes impuesto", "resultadoAI", acums.resultadoAI, "cf-ebitda-row")}
          ${dataRow("Impuesto estimado (solo si positivo)", m=>C[m]?.impuesto||0, acums.impuesto)}
          ${keyRow("Resultado neto", "resultado", acums.resultado, "cf-result-row")}

          ${spacer()}
          ${secRow("FINANCIAMIENTO — Recursos para ejecutar el proyecto")}
          ${subRow("Recursos propios")}
          ${dataRow("Aporte capital terreno", m=>C[m]?.apTerreno||0, acums.apTerreno)}
          ${dataRow("Aporte de capital",      m=>C[m]?.apCapital||0, acums.apCapital)}
          ${totRow("Total recursos propios",  "financPropios", acums.financPropios)}
          ${subRow("Recursos externos")}
          ${dataRow("Crédito hipotecario bancario", m=>mv(d.creditoHipotecario,m), acum(m=>mv(d.creditoHipotecario,m)))}
          ${totRow("Total recursos externos", "financExterno", acums.financExterno)}
          ${totRow("Total financiamiento", "financTotal", acums.financTotal, "cf-tot-row cf-financ-row")}

          ${spacer()}
          ${secRow("CONTROL DEL PROYECTO")}
          ${totRow("Costo total proyecto",     "costoTotal",  acums.costoTotal)}
          ${totRow("Financiamiento acumulado", "financTotal", acums.financTotal)}
          ${keyRow("Posición financiera del proyecto", "saldoProyecto", acums.saldoProyecto, "cf-cajaacum-row")}
...
async function saveIngreso() {
  ...
  if (typeof window.supabaseClient === "undefined") {
    showToast("⚠️ Sin conexión a Supabase."); return;
  }
  if (_editingIngresoId !== null) {
    const { error } = await window.supabaseClient
      .from("ingresos_junquillar")
      .update(entry)
      .eq("id", _editingIngresoId);
    if (error) { showToast(`Error: ${error.message}`); return; }
    showToast("✓ Ingreso actualizado");
  } else {
    const { error } = await window.supabaseClient
      .from("ingresos_junquillar")
      .insert({ ...entry, proyecto: PROJECT_NAME });
    if (error) { showToast(`Error: ${error.message}`); return; }
  ...
  renderVentas();
  renderKPIs();
  if (typeof renderControlFinanciero === "function") renderControlFinanciero();
}
async function deleteIngreso(id) {
  const r = ventasIngresos.find(x => x.id === id);
  if (!r || !confirm(`¿Eliminar el ingreso "${r.concepto}"?\nEsta acción no se puede deshacer.`)) return;
  if (typeof window.supabaseClient === "undefined") {
    showToast("⚠️ Sin conexión."); return;
  }
  const { error } = await window.supabaseClient
    .from("ingresos_junquillar")
    .delete()
...
/* ── CONTROL FINANCIERO: filtro de rango ──────────────────── */
let cfFiltroDesde = localStorage.getItem("junqo_cf_desde") || "";
let cfFiltroHasta = localStorage.getItem("junqo_cf_hasta") || "";


/* ── ELIMINAR ABONO DE CARTOLA ───────────────────────────── */
async function deleteMovBanco(id) {
  if (!confirm("¿Eliminar este abono de la cartola?\nSe eliminará de movimientos bancarios.")) return;
  if (typeof window.supabaseClient === "undefined") { showToast("⚠️ Sin conexión."); return; }
  const { error } = await window.supabaseClient
...
window.deleteMovBanco = deleteMovBanco;

/* ── PRESUPUESTOS POR CATEGORÍA ──────────────────────────── */
let presupuestos = []; // cargado desde Supabase

async function loadPresupuestos() {
  if (typeof window.supabaseClient === "undefined") return;
  const { data, error } = await window.supabaseClient
    .from("presupuestos_junquillar")
    .select("*")
    .eq("proyecto", PROJECT_NAME)
    .eq("activo", true)
    .order("id");
  if (error) { console.error("[Presupuestos]", error); return; }
  presupuestos = data || [];
  console.log(`[Presupuestos] ${presupuestos.length} categorías cargadas.`);
}

async function savePresupuesto(id, presupuesto_neto, observacion) {
  if (typeof window.supabaseClient === "undefined") return;
  const { error } = await window.supabaseClient
    .from("presupuestos_junquillar")
    .update({ presupuesto_neto, observacion })
    .eq("id", id);
  if (error) { showToast(`Error: ${error.message}`); return; }
  await loadPresupuestos();
  renderPresupuestos();
...
function renderPresupuestos() {
  ...
  for (const g of gastos) {
    ...
    gastosPorCat[cat] = (gastosPorCat[cat] || 0) + numberValue(g.neto);
  }

  const totalPres = presupuestos.reduce((s,p)=>s+numberValue(p.presupuesto_neto),0);
  const totalReal = presupuestos.reduce((s,p)=>s+(gastosPorCat[p.categoria]||0),0);
  const totalDif  = totalPres - totalReal;

  el.innerHTML = `
  <div class="kpi-grid" style="margin-bottom:20px">
    <div class="kpi-card">
      <div class="kpi-title">Presupuesto total</div>
      <div class="kpi-value">${formatoCLP(totalPres)}</div>
      <div class="kpi-footer">${presupuestos.filter(p=>p.activo).length} categorías activas</div>
    </div>
    <div class="kpi-card">
      <div class="kpi-title">Ejecutado neto</div>
      <div class="kpi-value" style="color:var(--red)">${formatoCLP(totalReal)}</div>
      <div class="kpi-footer">Gastos registrados sin IVA</div>
    </div>
    <div class="kpi-card">
      <div class="kpi-title">${totalDif>=0?"Saldo disponible":"Sobrecosto"}</div>
      <div class="kpi-value" style="color:${totalDif>=0?"var(--green)":"var(--red)}"}">
        ${totalDif<0?"-":""}${formatoCLP(Math.abs(totalDif))}
      </div>
      <div class="kpi-footer">Presupuesto − ejecutado</div>
    </div>
    <div class="kpi-card">
      <div class="kpi-title">Avance general</div>
      <div class="kpi-value">${totalPres>0?(totalReal/totalPres*100).toFixed(1)+"%":"—"}</div>
      <div class="kpi-footer">% del presupuesto ejecutado</div>
    </div>
  </div>
  ...
  <div class="card">
    <div class="card-header-row">
      <div>
        ...
        <div class="card-sub">Edita directamente el monto. Los valores alimentan Resumen, Control Financiero y Reportes.</div>
      </div>
      <button class="jv-nuevo-btn" onclick="mostrarFormNuevaCategoria()">+ Agregar categoría</button>
    </div>

    <!-- Formulario nueva categoría (oculto por defecto) -->
    <div id="pres-nuevo-form" style="display:none;margin:14px 0;padding:14px;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0">
      <div style="display:grid;grid-template-columns:1fr 140px 1fr auto;gap:8px;align-items:end">
        <div>
          <label style="font-size:11px;color:var(--muted);display:block;margin-bottom:4px">Categoría</label>
          <input id="pres-cat-nombre" type="text" class="filter-select" style="width:100%" placeholder="Ej: Terreno, Materiales…"/>
        </div>
        <div>
          <label style="font-size:11px;color:var(--muted);display:block;margin-bottom:4px">Presupuesto neto</label>
          <input id="pres-cat-monto" type="number" class="filter-select" style="width:100%" placeholder="0" min="0"/>
        </div>
        <div>
          <label style="font-size:11px;color:var(--muted);display:block;margin-bottom:4px">Observación</label>
          <input id="pres-cat-obs" type="text" class="filter-select" style="width:100%" placeholder="Opcional"/>
        </div>
        <div style="display:flex;gap:6px">
          <button class="jv-nuevo-btn" onclick="guardarNuevaCategoria()">Guardar</button>
          <button class="filter-btn-clear" style="margin:0" onclick="ocultarFormNuevaCategoria()">✕</button>
        </div>
      </div>
    </div>

    <div class="table-wrap" style="margin-top:14px">
      <div class="table-head" style="grid-template-columns:1fr 150px 140px 90px 70px 140px 60px">
        <div>Categoría</div>
        <div style="text-align:right">Presupuesto neto</div>
      ...
      <div>
        ${presupuestos.length === 0
          ? `<div class="empty-state">Sin categorías. Usa "+ Agregar categoría" para comenzar.</div>`
          : presupuestos.map(p => {
            const real = gastosPorCat[p.categoria] || 0;
            const pres = numberValue(p.presupuesto_neto);
            const dif  = pres - real;
            const pct  = pres > 0 ? (real/pres*100) : 0;
            const barW = Math.min(100, pct).toFixed(0);
            const barClr = pct > 100 ? "var(--red)" : pct > 80 ? "var(--amber)" : "var(--green)";
            const difClr = dif >= 0 ? "var(--green)" : "var(--red)";
            return `
            <div class="table-row" style="grid-template-columns:1fr 150px 140px 90px 70px 140px 60px">
              <div>
                <div style="font-weight:600;font-size:13px">${p.categoria}</div>
                ${p.observacion?`<div style="font-size:11px;color:var(--muted)">${p.observacion}</div>`:""}
                <div style="margin-top:5px;height:4px;background:#e2e8f0;border-radius:2px;width:140px">
                  <div style="height:4px;background:${barClr};border-radius:2px;width:${barW}%"></div>
                </div>
              </div>
              <div style="text-align:right">
                <input type="number" class="filter-select"
                  style="width:130px;text-align:right;font-size:12px"
                  value="${pres||""}"
                  placeholder="Sin definir"
                  onblur="savePresupuesto(${p.id}, Number(this.value)||0, '${(p.observacion||"").replace(/'/g,"")}')"
                />
              </div>
              <div style="text-align:right;font-size:13px">${formatoCLP(real)}</div>
              <div style="text-align:right;font-size:13px;color:${barClr};font-weight:600">${pres>0?pct.toFixed(1)+"%":"—"}</div>
              <div>
                <button class="jv-badge"
                  style="background:${p.activo?"var(--green-soft)":"#f8fafc"};color:${p.activo?"var(--green)":"var(--muted)"};border:none;cursor:pointer"
                  onclick="togglePresupuestoActivo(${p.id},${!p.activo})">
                  ${p.activo?"Activo":"Inactivo"}
                </button>
              </div>
              <div style="text-align:right;font-weight:700;color:${difClr}">${dif<0?"-":""}${formatoCLP(Math.abs(dif))}</div>
              <div>
                <button class="action-btn" title="Eliminar" onclick="eliminarPresupuesto(${p.id})">🗑️</button>
              </div>
            </div>`;
          }).join("")}
      </div>
...
function mostrarFormNuevaCategoria() {
  const f = $("pres-nuevo-form");
  if (f) f.style.display = "block";
}
function ocultarFormNuevaCategoria() {
  const f = $("pres-nuevo-form");
  if (f) f.style.display = "none";
}

async function guardarNuevaCategoria() {
  const cat = ($("pres-cat-nombre")?.value||"").trim();
  const monto = Number($("pres-cat-monto")?.value||0);
  const obs   = ($("pres-cat-obs")?.value||"").trim();
  if (!cat) { showToast("⚠️ Ingresa el nombre de la categoría."); return; }
  if (typeof window.supabaseClient === "undefined") return;
  const { error } = await window.supabaseClient
    .from("presupuestos_junquillar")
    .insert({ categoria:cat, presupuesto_neto:monto, observacion:obs||null, proyecto:PROJECT_NAME });
  if (error) { showToast(`Error: ${error.message}`); return; }
  ocultarFormNuevaCategoria();
  await loadPresupuestos();
  renderPresupuestos();
  showToast("✓ Categoría agregada");
}

async function togglePresupuestoActivo(id, activo) {
  if (typeof window.supabaseClient === "undefined") return;
  const { error } = await window.supabaseClient
    .from("presupuestos_junquillar")
    .update({ activo })
...
window.mostrarFormNuevaCategoria  = mostrarFormNuevaCategoria;
window.ocultarFormNuevaCategoria  = ocultarFormNuevaCategoria;
window.guardarNuevaCategoria      = guardarNuevaCategoria;
window.togglePresupuestoActivo    = togglePresupuestoActivo;
window.eliminarPresupuesto        = eliminarPresupuesto;


window.renderPresupuestos = renderPresupuestos;
window.savePresupuesto    = savePresupuesto;
...
async function loadIngresos() {
  ...
  if (error) { console.error("[Ingresos]", error); return; }
  ventasIngresos = (data || []).map(r => ({
    id:                r.id,
    fecha:             r.fecha,
    concepto:          r.concepto || "",
    tipo:              r.tipo || "",
    categoria_contable:r.categoria_contable || "Aporte",
    monto:             r.monto || 0,
  ...
  console.log(`[Ingresos] ${ventasIngresos.length} ingresos cargados.`);
}

/* ── APORTES E INGRESOS RENDER ───────────────────────────── */
function renderVentas(){
  const el = $("ventas-root");
  if(!el) return;

  // Clasificaciones de abonos que corresponden a Aportes e Ingresos
  // Clasificaciones que van a Aportes e Ingresos (explícitas, sin default)
  const CLASIF_CAPITAL = ["Aporte de capital","Anticipo","Venta","Préstamo recibido"];

  // Solo abonos con clasificación explícita de capital — sin asumir nada
  const abonosBanco = (typeof movBanco !== "undefined" ? movBanco : [])
    .filter(m => m.tipo === "abono" && m.clasificacion && CLASIF_CAPITAL.includes(m.clasificacion))
    .map(m => ({
      id:                m.id,
      fecha:             m.fecha,
      concepto:          m.descripcion || "Abono bancario",
      categoria_contable: m.clasificacion,
      monto:             m.monto,
      estado:            "Recibido",
      _fromBanco:        true,
    }));

  // Merge: manuales + bancarios clasificados — sin duplicados por fecha+monto
  ...
  <div class="card">
    <div class="card-header-row">
      <div>
        <div class="card-title">Registro de aportes e ingresos</div>
        <div class="card-sub">Capital aportado, anticipos y venta del proyecto</div>
      </div>
      <button class="jv-nuevo-btn" onclick="openModalIngreso()">+ Nuevo ingreso</button>
    </div>
    <div class="table-wrap" style="margin-top:14px">
      <div class="table-head jv-ing-head">
        <div>Fecha</div><div>Concepto</div><div>Tipo</div>
              ...
              <div class="doc-actions">
                ${r._fromBanco
                  ? `<span class="jv-badge" style="background:#eff6ff;color:#1d4ed8;font-size:10px">🏦 Banco</span>
                     <button class="action-btn" title="Eliminar abono" onclick="deleteMovBanco(${r.id})">🗑️</button>`
...
const cfgUsers = []; // Sin usuarios hardcodeados — agregar usuarios reales desde la app
const cfgCategorias = {
      ...
      <div class="cfg2-users">
        ${cfgUsers.length === 0
          ? `<div class="empty-state" style="padding:24px;text-align:center;color:var(--muted)">
               ...
               <div style="font-weight:600;margin-bottom:4px">No hay usuarios registrados</div>
               <div style="font-size:12px">Usa el botón "Agregar usuario" para añadir uno.</div>
             </div>`
