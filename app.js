...
function renderMovTabla(){
  ...
  // Movimientos banco → impactan banco, no IVA
  for(const m of banco){
    rows.push({
      fecha:       m.fecha,
      tipo:        m.tipo==="abono"?"Ingreso":"Pago",
...
function renderControlFinanciero() {
  ...
  for(const m of CF_KEYS){
    ...
    const impuesto    = resultadoAI > 0 ? mv(d.impuesto,m) : 0; // solo si positivo
  ...
  const kpiIngr    = visKeys.reduce((s,m)=>s+C[m].ingresos,   0);
  const kpiCosto   = visKeys.reduce((s,m)=>s+C[m].costoTotal, 0);
  const kpiRes     = visKeys.length ? C[visKeys[visKeys.length-1]].resultado : 0;
  const kpiFin     = visKeys.reduce((s,m)=>s+C[m].financTotal,0);
  const kpiSaldo   = visKeys.length ? C[visKeys[visKeys.length-1]].saldoProyecto : 0;
  ...
  const subRow = lbl => `<tr class="cf-sub-row"><td colspan="${nCols}" style="font-size:11px;color:var(--muted);padding:4px 12px">${lbl}</td></tr>`;
  ...
  <div class="card">
    <div class="card-header-row" style="flex-wrap:wrap;gap:12px">
      <div>
        ...
        <div class="card-sub">Análisis del proyecto: ¿Va bien o mal?</div>
      </div>
    ...
    <div class="cf-kpi-grid">
      <div class="cf-kpi-card">
        ...
        <div class="cf-kpi-val" style="color:var(--brand)">${fmtKpi(kpiIngr)}</div>
        <div class="cf-kpi-sub">Venta casa + otros</div>
      </div>
      <div class="cf-kpi-card">
        <div class="cf-kpi-lbl">Costo total proyecto</div>
        <div class="cf-kpi-val" style="color:var(--red)">${fmtKpi(kpiCosto)}</div>
        <div class="cf-kpi-sub">Directo + administrativo</div>
      </div>
      <div class="cf-kpi-card">
        <div class="cf-kpi-lbl">Resultado neto</div>
        <div class="cf-kpi-val" style="color:${kpiClr(kpiRes)}">${fmtKpi(kpiRes)}</div>
        <div class="cf-kpi-sub">Después de impuesto</div>
      </div>
      <div class="cf-kpi-card">
        <div class="cf-kpi-lbl">Control del proyecto</div>
        <div class="cf-kpi-val" style="color:${kpiClr(kpiSaldo)}">${fmtKpi(kpiSaldo)}</div>
    ...
    <div class="cf-table-wrap">
      <table class="cf-table">
        ...
        <tbody>
          ...
          ${spacer()}
          ${keyRow("EBITDA / Resultado antes impuesto", "resultadoAI", "cf-ebitda-row")}
          ${dataRow("Impuesto estimado (solo si positivo)", m => C[m]?.impuesto||0)}
          ${keyRow("Resultado neto",                        "resultado",   "cf-result-row")}
          ...
          ${dataRow("Aporte de capital",      m => C[m]?.apCapital||0)}
          ${totRow("Total recursos propios",  "financPropios")}
          ${subRow("Recursos externos")}
          ${dataRow("Crédito hipotecario bancario", m=>mv(d.creditoHipotecario,m))}
          ${totRow("Total recursos externos", "financExterno")}
          ${totRow("Total financiamiento",    "financTotal", "cf-tot-row cf-financ-row")}

          ${spacer()}
          ${secRow("CONTROL DEL PROYECTO")}
          ${totRow("Costo total proyecto",   "costoTotal")}
          ${totRow("Total financiamiento",   "financTotal")}
          ${keyRow("Saldo proyecto (excedente / por financiar)", "saldoProyecto", "cf-cajaacum-row")}
