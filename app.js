...
function renderControlFinanciero() {
  ...
  // ── Recopilar todos los meses reales con datos ─────────────
  ...
  for (const ing of (typeof ventasIngresos !== "undefined" ? ventasIngresos : [])) {
    const m = (fechaOrdenable(ing.fecha) || "").slice(0, 7);
    if (m) mesesSet.add(m);
  }
  for (const mov of (typeof movBanco !== "undefined" ? movBanco : [])) {
    if (mov.tipo === "abono") {
      const m = (fechaOrdenable(mov.fecha) || "").slice(0, 7);
      if (m) mesesSet.add(m);
    }
  }

  if (mesesSet.size === 0) {
    el.innerHTML = `<div class="card"><div class="empty-state">Sin datos registrados. Carga gastos o aportes para ver el flujo financiero.</div></div>`;
    return;
  }

  // ── Filtro de rango configurable ───────────────────────────
  const hoy = new Date();
  const mesHoy = `${hoy.getFullYear()}-${String(hoy.getMonth()+1).padStart(2,"0")}`;
  const mesesArr = [...mesesSet].sort();
  const minMes = mesesArr[0];
  const maxMes = mesesArr[mesesArr.length-1] > mesHoy ? mesesArr[mesesArr.length-1] : mesHoy;

  // Usar filtro guardado o rango completo por defecto
  const primerMes = cfFiltroDesde || minMes;
  const ultimoMes = cfFiltroHasta || maxMes;

  // Generar todos los meses entre primero y último
  const ms = [];
  let cur = primerMes;
  while (cur <= ultimoMes) {
    ms.push(cur);
    ...
    cur = next;
    if (ms.length > 36) break; // máximo 3 años
  }

  // ── Label de mes: "Dic 2025", "Ene 2026" ──────────────────
  const MESES_LABEL = ["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"];
  const mesLabel2 = ym => {
    const [y, mo] = ym.split("-");
    return `${MESES_LABEL[Number(mo)-1]} ${y}`;
  };
  ...
  for (const g of gastos) {
    ...
    gastosPorMes[m][cat] = (gastosPorMes[m][cat] || 0) + numberValue(g.neto);
  }
  ...
  for (const ing of (typeof ventasIngresos !== "undefined" ? ventasIngresos : [])) {
    ...
    const m = (fechaOrdenable(ing.fecha) || "").slice(0, 7);
    if (!m) continue;
    aportesPorMes[m] = (aportesPorMes[m] || 0) + numberValue(ing.monto);
  }
  for (const mov of (typeof movBanco !== "undefined" ? movBanco : [])) {
    if (mov.tipo !== "abono") continue;
    const m = (fechaOrdenable(mov.fecha) || "").slice(0, 7);
    if (!m) continue;
    aportesPorMes[m] = (aportesPorMes[m] || 0) + numberValue(mov.monto);
  }
  ...
  const C = {};
  for (const m of ms) {
    const gastoTotal = categorias.reduce((s, cat) => s + (gastosPorMes[m]?.[cat] || 0), 0);
    const aporte     = aportesPorMes[m] || 0;
    const flujo      = aporte - gastoTotal;
    cajaAcum        += flujo;
    C[m] = { gastoTotal, aporte, flujo, cajaAcum };
  }

  // ── KPIs ──────────────────────────────────────────────────
  const totGasto  = ms.reduce((s, m) => s + C[m].gastoTotal, 0);
  const totAporte = ms.reduce((s, m) => s + C[m].aporte, 0);
  const totFlujo  = totAporte - totGasto;
  const cajaFinal = C[ms[ms.length-1]].cajaAcum;

  const fmt = v => {
    if (!v || v === 0) return `<span class="cf-zero">$0</span>`;
    const s = Math.abs(Math.round(v)).toLocaleString("es-CL");
    return v < 0
      ? `<span class="cf-neg">-$${s}</span>`
      : `<span class="cf-pos">$${s}</span>`;
  };
  const fmtKpi = v => {
    const s = Math.abs(Math.round(v)).toLocaleString("es-CL");
    return v < 0 ? `-$${s}` : `$${s}`;
  };
  const clr = v => v >= 0 ? "var(--brand)" : "var(--red)";
  ...
  const dataRow = (lbl, fn, cls="") =>
    ...
    ms.map(m => `<td class="cf-num">${fmt(fn(m))}</td>`).join("") +
    `</tr>`;
  const totRow = (lbl, fn, cls="") =>
    `<tr class="cf-tot-row${cls?" "+cls:""}">` +
    `<td class="cf-lbl">${lbl}</td>` +
    ms.map(m => `<td class="cf-num">${fmt(fn(m))}</td>`).join("") +
    `</tr>`;
  ...
  <div class="card">
    <div class="card-header-row">
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--muted)">
          <span>Desde</span>
          <input type="month" class="filter-select" style="height:34px;font-size:12px"
            value="${cfFiltroDesde || minMes}"
            min="${minMes}" max="${maxMes}"
            onchange="cfFiltroDesde=this.value;localStorage.setItem('junqo_cf_desde',this.value);renderControlFinanciero()"/>
          <span>Hasta</span>
          <input type="month" class="filter-select" style="height:34px;font-size:12px"
            value="${cfFiltroHasta || maxMes}"
            min="${minMes}" max="${maxMes}"
            onchange="cfFiltroHasta=this.value;localStorage.setItem('junqo_cf_hasta',this.value);renderControlFinanciero()"/>
          <button class="filter-btn-clear" style="margin:0;height:34px;font-size:11px"
            onclick="cfFiltroDesde='';cfFiltroHasta='';localStorage.removeItem('junqo_cf_desde');localStorage.removeItem('junqo_cf_hasta');renderControlFinanciero()">↺</button>
        </div>
      </div>
    </div>
    ...
    <div class="cf-kpi-grid">
      <div class="cf-kpi-card">
        <div class="cf-kpi-lbl">Gastos netos acumulados</div>
        <div class="cf-kpi-val" style="color:var(--red)">${fmtKpi(totGasto)}</div>
        <div class="cf-kpi-sub">${gastos.length} documentos registrados</div>
      </div>
      <div class="cf-kpi-card">
        <div class="cf-kpi-lbl">Aportes recibidos</div>
        <div class="cf-kpi-val" style="color:var(--green)">${fmtKpi(totAporte)}</div>
        <div class="cf-kpi-sub">Ingresos + abonos bancarios</div>
      </div>
      <div class="cf-kpi-card">
        <div class="cf-kpi-lbl">Caja acumulada</div>
        <div class="cf-kpi-val" style="color:${clr(cajaFinal)}">${fmtKpi(cajaFinal)}</div>
      ...
      <table class="cf-table">
        <thead>
          <tr>
            ...
            ${ms.map(m => `<th class="cf-th-month">${mesLabel2(m)}</th>`).join("")}
          </tr>
        </thead>
        <tbody>
          ${secRow("GASTOS OPERACIONALES — por categoría")}
          ${categorias.map(cat =>
            dataRow(cat, m => gastosPorMes[m]?.[cat] ? -gastosPorMes[m][cat] : 0)
          ).join("")}
          ${totRow("Total gastos", m => -C[m].gastoTotal, "cf-tot-row")}

          ${spacer()}
          ${secRow("FINANCIAMIENTO — aportes e ingresos")}
          ${dataRow("Aporte de capital", m => C[m].aporte)}
          ${spacer()}

          ${totRow("Flujo neto del mes", m => C[m].flujo, "cf-result-row")}
          ${totRow("Caja acumulada",     m => C[m].cajaAcum, "cf-cajaacum-row")}
        </tbody>
      </table>
    </div>
  </div>`;
