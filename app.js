...
function renderControlFinanciero() {
  ...
  <style>
    ...
    /* Primera columna Concepto sticky horizontal */
    .cf-lbl {
      position: sticky;
      left: 0;
      background: white;
    ...
    /* Columna Acumulado sticky junto a Concepto */
    .cf-acum-cell {
      background: #f0fdf4;
    ...
    /* Filas de sección: primera celda sticky */
    .cf-sec-row td:first-child,
    .cf-sub-row td:first-child,
    .cf-spacer-row td:first-child,
    .cf-tot-row td:first-child,
    .cf-data-row td:first-child,
    .cf-ebitda-row td:first-child,
    .cf-result-row td:first-child,
    .cf-cajaacum-row td:first-child,
    .cf-financ-row td:first-child {
      position: sticky;
      left: 0;
      z-index: 2;
    }
    /* Backgrounds para filas especiales */
    .cf-sec-row td { background: #f1f5f9; }
    .cf-tot-row td:first-child { background: #f8fafc; }
    .cf-ebitda-row td:first-child { background: #eff6ff; }
    .cf-result-row td:first-child { background: #f0fdf4; }
    .cf-cajaacum-row td:first-child { background: #fefce8; }
  </style>
...
function renderPresupuestos() {
  ...
  <div class="kpi-grid" style="margin-bottom:20px">
    <div class="kpi-card">
      ...
      <div class="kpi-footer">${presupuestos.length} categorías</div>
    </div>
    <div class="kpi-card">
    ...
    <div class="kpi-card">
      <div class="kpi-title">Avance</div>
  ...
  <div class="card">
    ...
    <div class="card-sub">Edita el presupuesto neto de cada partida. Haz clic en el monto para editar.</div>
    <div class="table-wrap" style="margin-top:14px">
      <div class="table-head" style="grid-template-columns:1fr 140px 140px 100px 80px 140px">
        <div>Categoría</div>
      ...
      <div>
        ${presupuestos.map(p => {
          ...
          const clr  = dif >= 0 ? "var(--green)" : "var(--red)";
          const barW = Math.min(100, pct).toFixed(0);
          const barClr = pct > 100 ? "var(--red)" : pct > 80 ? "var(--amber)" : "var(--green)";
          return `
          <div class="table-row" style="grid-template-columns:1fr 140px 140px 100px 80px 140px">
            <div>
              ...
              <div style="margin-top:4px;height:4px;background:#e2e8f0;border-radius:2px;width:120px">
            ...
            <div style="text-align:right">
              <input type="text" class="filter-select"
                ...
                value="${pres>0?Math.round(pres).toLocaleString("es-CL"):""}"
                placeholder="Sin definir"
                onblur="savePresupuesto(${p.id}, Number(this.value.replace(/\./g,'').replace(',','.'))||0, '${(p.observacion||"").replace(/'/g,"")}')"
              />
            </div>
            <div style="text-align:right;font-size:13px">${formatoCLP(real)}</div>
            <div style="text-align:right;font-size:13px;color:${barClr}">${pct.toFixed(1)}%</div>
            <div>
              <span class="jv-badge" style="background:${pres>0?"var(--green-soft)":"#f8fafc"};color:${pres>0?"var(--green)":"var(--muted)"}">
                ${pres>0?"Definido":"Pendiente"}
              </span>
            </div>
            <div style="text-align:right;font-weight:600;color:${clr}">${dif<0?"-":""}${formatoCLP(Math.abs(dif))}</div>
          </div>`;
        }).join("")}
      </div>
    </div>
...
function renderVentas(){
  ...
  const CLASIF_CAPITAL = ["Aporte de capital","Anticipo","Venta","Préstamo recibido","Aporte terreno"];

  // Solo abonos bancarios clasificados como capital/ingreso de proyecto
  const abonosBanco = (typeof movBanco !== "undefined" ? movBanco : [])
    .filter(m => m.tipo === "abono" && CLASIF_CAPITAL.includes(m.clasificacion || "Aporte de capital"))
    .map(m => ({
      id:                m.id,
      fecha:             m.fecha,
      concepto:          m.descripcion || "Abono bancario",
      categoria_contable: m.clasificacion || "Aporte de capital",
      monto:             m.monto,
      estado:            "Recibido",
      _fromBanco:        true,
    }));
...
const cfgUsers = [
  // Solo el administrador real — los demás eran datos de ejemplo
