  ...
  <div class="card">
    <div class="card-header-row">
      ...
      <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
        ...
        <select class="filter-select" style="height:30px;font-size:11px"
          onchange="concMovFiltro=this.value;renderConciliacion()">
    ...
    <div class="table-wrap" style="margin-top:14px">
      <div class="table-head conc-movs-head" style="grid-template-columns:100px 1fr 130px 80px 160px 90px">
        <div>Fecha</div>
      ...
      <div>
        ${[...cuad.movMes]
          ...
          .filter(m => {
            if (!concMovFiltro) return true;
            if (concMovFiltro === "cargo") return m.tipo === "cargo";
            if (concMovFiltro === "abono") return m.tipo === "abono";
            if (concMovFiltro === "sin_clasificar") return m.tipo === "abono" && !m.clasificacion;
            return m.clasificacion === concMovFiltro;
          })
          .map(m=>`
          <div class="table-row conc-mov-row" style="grid-template-columns:100px 1fr 130px 80px 160px 90px">
            ...
            <div>
              ${m.tipo==="abono" ? `
                <select class="filter-select" style="font-size:11px;height:28px;padding:2px 6px"
                  onchange="clasificarAbono(${m.id},this.value)">
                  <option value="" ${!m.clasificacion?" selected":""}>Sin clasificar</option>
                  ${["Aporte de capital","Anticipo","Venta","Préstamo recibido","Arriendo","Devolución","Reembolso","Otro ingreso"]
                    .map(c=>`<option value="${c}"${m.clasificacion===c?" selected":""}>${c}</option>`).join("")}
                </select>` : `<span style="color:var(--muted);font-size:11px">—</span>`}
            </div>
            <div>
              <span class="jv-badge" style="background:var(--green-soft);color:var(--green)">${m.estado||"Recibido"}</span>
            </div>
          </div>`).join("")}
      </div>
    </div>
...
/* ── Exponer globales ─────────────────────────────────────── */
/* ── Clasificar abono bancario ────────────────────────────── */
async function clasificarAbono(id, clasificacion) {
...
window.clasificarAbono = clasificarAbono;

let concMovFiltro = "";
window.concMovFiltro = concMovFiltro;
